...
| 原生模块崩溃 | 各种子进程 | minidump + 崩溃上报（见[监控章节](/part3-engineering/23-observability)） |
...
深度分析方法见[案例库 · GPU 崩溃五步分析法](/cases/02-gpu-crash)。
...
输出 Chromium 层 + 主进程 console 到文件。配合 `--v=1` 可提级 verbosity。排查「日志回捞」体系见[监控章节](/part3-engineering/23-observability)。
