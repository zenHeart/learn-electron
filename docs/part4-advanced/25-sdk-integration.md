...
这是「渲染层热更新」（见[更新章节](/part3-engineering/21-releases-updates)）可行的前提——渲染层产物不含任何原生依赖，才能被独立替换。
...
- 命令行开关注入运行时行为（feature flag / GPU 规则）：见[命令行速查](/appendix/cli-reference)与[性能章节](/part3-engineering/18-performance)
