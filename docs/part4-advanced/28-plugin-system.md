...
模式讲完了，看两个真实样本——它们都来自壳 + Web 架构的大型客户端，恰好分别落在「插件形态」与「版本治理」两端，一个回答「插件的价值到底锚在哪」，一个回答「版本对不上时拿什么对账」。

**样本一：空壳 install 的形态观。** 这个客户端的某个「插件包」解包后，install 入口只有 7 行——全是 TODO 注释，没有任何执行逻辑，而它稳定运行多年。乍看是敷衍，拆开看是想明白了：插件的价值锚点从来不是 install 里的代码量，而是两件更上层的事——

- **统一挂载点**：业务页通过 `app.use(plugin)` 声明依赖，壳层因此拿到「这张页面用了哪个插件、该推哪个版本」的完整信息——`app.use` 这一行，就是业务页在整条分发链路上的升级锚点；
- **更新分发**：升级、灰度、回滚都沿挂载关系推送，插件包本身只是分发单位，不必在 install 里做任何事。

反过来，install 里写几百行初始化逻辑，插件与宿主的耦合只会更深，分发链路反而更不敢动。空壳 install 是个极端样本，但它把一件事拍实了：**评估一个插件系统，看它的挂载与分发协议，不要看 install 写了多少行**。

**样本二：DOM 标签版本自省。** 壳加载 web 资源时，把资源版本号写进页面里一个约定的 DOM 标签：`<div id="app-tag" data-version="2.3.1@a1b2c3">`。前端任何代码都能零依赖地抠出当前版本，与远端配置对比：

```js
// 5 行版本自省：不需要构建时注入，不需要额外 IPC
const local = document.querySelector('#app-tag')?.dataset.version
if (local && remote.latest !== local) {
  report('version-mismatch', { local, remote: remote.latest })
}
```

版本串用 `x.y.z@hash` 双段不是随手写的：语义化版本给人读、给灰度策略用，资源哈希给机器对账——版本号相同而哈希不同，就是「半截热更」的事故现场，单独看版本号永远发现不了。

这一招在热更架构里价值放大。该客户端的 web 资源与壳本体（asar）走**双通道热更**：web 资源高频迭代，走独立下发通道；壳本体低频变更，走整包升级通道——两条通道的版本节奏完全不同（分层设计见[发布与更新](/part3-engineering/21-releases-updates)）。两套版本各自独立演进，出问题时「页面版本 × 壳版本」的组合矩阵靠翻日志反推几乎不可能；有了 DOM 标签，现场打开 DevTools 抠一下就能对账。

两个样本共同指向一件事：插件系统里真正值钱的是**协议与锚点**——挂载协议决定升级能不能推下去，版本锚点决定出了事能不能对上账。

install 里的代码量，在两者面前都是次要变量。
