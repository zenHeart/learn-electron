...
多账号应用（多身份工作台、多店铺运营）的核心存储决策：**每个身份一个 session 分区**——cookie、localStorage、IndexedDB、缓存全部按隔离：

```js
const webPreferences = {
  partition: `persist:account-${userId}`   // persist: 前缀 = 持久化分区（落 userData/Partitions/）
}
// 不带 persist: 前缀 = 内存分区，应用退出即清空（一次性会话/隐私窗口用）
```

分区决定了渲染层 Web 存储的「户籍」：同一个页面在 `partition:account-1` 和 `account-2` 下看到的是两套完全独立的 localStorage 与 cookie。登录态注入也走分区级 API：

```js
// 主进程：把已保存的凭证写进对应分区的 session（复用 Web 登录态的桌面壳常用姿势）
const ses = session.fromPartition(`persist:account-${userId}`)
await ses.cookies.set({ url: 'https://app.example.com', name: 'auth', value: token, httpOnly: true })

// 退出登录：逐条移除（精确）或 clearStorageData（彻底）
await ses.cookies.remove('https://app.example.com', 'auth')
```

::: pitfall 坑位警报
①凭证落盘的安全权衡：`persist:` 分区的 cookie 明文存在 userData 下，敏感 token 应存 [safeStorage](#safestorage-凭据加密的完整示例) 加密后按需注入内存分区，而不是明文 cookie 常驻；②分区的命名一旦上线就是用户数据契约——改名等于所有账号「集体掉线」，命名要带版本意识（如 `account-v2-${id}`）。
:::
...
它是**原生模块**（Node API 级别绑定 V8），对 Electron 版本的 ABI 敏感：换 Electron 版本必须重新编译，集成与打包要点见[原生能力扩展](/part4-advanced/25-sdk-integration)。
...
典型症状：某个页面（或某台机器上的所有页面）打开即缩放异常、布局崩坏甚至白屏，而你的代码里"没有任何缩放逻辑"——脏数据在 Preferences 里，不在代码里。完整事故复盘见[案例：缩放持久化白屏](/cases/01-zoom-white-screen)。
