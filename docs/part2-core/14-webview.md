...
## 先建立一个认知锚点：BrowserWindow vs 内嵌视图

初学者最常见的困惑（也是检验是否真正理解进程模型的试金石）：**BrowserWindow 和内嵌视图（WebContentsView，旧 API 叫 BrowserView）的本质区别是什么？**

```text
BrowserWindow   = 原生窗口句柄 + 一个渲染进程（操作系统任务栏里的一项）
WebContentsView = 纯渲染区域，没有窗口身份（必须依附某个 BrowserWindow/BaseWindow）
```

一句话：**要出现在任务栏/可独立拖动 → BrowserWindow；要在窗口内部划出一块独立页面区域（互不影响的进程、不同权限）→ 内嵌视图**。历史上 BrowserView 承担这个职责，Electron 30 起废弃并统一到 WebContentsView（与 BaseWindow 组合，职责更清晰：窗口壳与内容分离）。下面的选型都以这三者 + iframe 展开。
...
::: exp
**1. WebContentsView 坐标系与平台可用区补偿。** `setBounds` 的原点相对**窗口内容区左上角、y 轴向下**，这与屏幕坐标（`screen` 系 API）原点在主屏左上角是两套体系，混用必错。布局时按平台补偿顶部：mac 无边框窗含红绿灯按钮区（约 28pt，或按 `trafficLightPosition` 实际值），Windows 自绘标题栏按自己设计稿高度算；一切高度计算收敛进唯一的 `layout()` 函数，`resize` 事件里只调它。坐标系差异的完整拆解见[案例：坐标系偏移](/cases/05-browserview-coords)。
