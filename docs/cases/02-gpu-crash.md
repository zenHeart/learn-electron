...
采集配置见[调试章节 · GPU trace](/part3-engineering/16-debugging)（`record-continuously` 循环覆盖模式 + memory dump），用 Perfetto 打开崩溃前 30 秒的 trace。
...
5. 兜底（见[性能章节](/part3-engineering/18-performance)）：不能升级 Chromium 的存量客户端，用远程规则引擎对问题机型下发 GPU flag（如禁用 DirectComposition 视频叠加）。

### 本案例的具象化：MPO（Multi-Plane Overlay）这个真实驱动级 bug

「视频 + 滚动 = 花屏」在 Windows 上有一个高频实锤根因：**MPO（多平面叠加）**——显卡驱动把视频帧直接叠到显示平面的优化路径，与 Chromium 的合成器配合有已知缺陷。社区证据链（AMD 用户社区反馈、Intel 论坛的滚动卡顿报告、NVIDIA 特定驱动版本问题）表明**三家显卡厂商都在不同时期踩过 MPO 的坑**，症状高度一致：视频区域花屏/绿屏/黑块、滚动时闪烁、特定驱动版本集中爆发。

对应的开关是一组（注意形态不同、以实测为准）：

```text
--disable-direct-composition-video-overlays      # 命令行开关形态（最常用）
--disable_direct_composition_video_overlays=1    # 环境变量形态
--disable-features=DirectCompositionVideoOverlays # feature 形态
```

三步查证（见[案例七](/cases/07-flag-not-working)）：先确认你的 Electron 版本对应的 Chromium 是否支持该开关（例：Chromium 108 支持前两种形态）、再核对注入时机在 `app.ready` 之前、最后在目标机型实测（`chrome://gpu` 页面确认 overlay 状态变化）。对存量客户端，这类开关正是[配置系统](/part2-core/13-config)「规则引擎」的典型下发对象——按 gpuVendorId + 驱动版本区间精准命中，灰度放量，观察崩溃率回落后再固化。
